...
**IMPORTANT:** The `convertToR()` function is not thread-safe. Do not simultaneously call `convertToR()` from different threads in the same process. A thread-safe alternative is presented in the R documentation for `convertToRlowLevel()`.
...
**IMPORTANT:** If an R array is converted to Java using a particular array order, use the same array order when converting it back from Java to R. Otherwise, the data will be in the wrong order.
...
The `convertToR()` function attempts to convert Java objects implementing the `java.util.Map` interface to named lists or data frames. The map keys must be string values. If the map contains same-length arrays, or same-length collections that can be converted to arrays, the map will be converted to a data frame. Otherwise, the object will be converted to a named list.
...
Java objects implementing the `java.util.Collection` interface can be converted to vectors, n-dimensional arrays, data frames, and unnamed lists. Conversions rules are dilineated in the table below. The `convertToR()` parameters `array.order` and `strings.as.factors` behave the same way for objects converted from collections as they do elsewhere. See [R Matrices and N-dimensional Arrays](#r_n_dimensional_arrays) for a description of the `array.order` parameter. Refer to [Java Maps](#java_maps) for details relating to `strings.as.factors`.

**NOTE:** If `lst` is an R list, `convertToR(convertToJava(lst))` may not result in a list. See (Conversion Issues)(#conversion_issues) for more information.
...
Uses generic Java objects. Here are some scenarios to be mindful of.

list(factor)

column major df with one column vs. row major df.

changing the length of something will result in a list being returned.

changing a value in a JS array (collection) will return a different data type.

c(1,2,3) will be an integer (1,2,3) coming back from JS.

As noted above, the `convertToJava()` function converts unnamed R lists to `ArrayList` objects which implement the `Collection` interface. The `convertToR()` function converts Java collections to the most appropriate R object depending on content. For example, a collection containing compatible scalars will be converted to an R vector, and a collection containing n-dimensional arrays will be converted to an (n + 1)-dimensional array. See [Java Collections](#java_collections) for more information.

**NOTE:** If `lst` is an R list, `convertToR(convertToJava(lst))` may not result in a list. As noted above, the `convertToJava()` function converts unnamed R lists to `ArrayList` objects which implement the `Collection` interface. The `convertToR()` function converts Java collections to the most appropriate R object depending on content. For example, a collection containing compatible scalars will be converted to an R vector, and a collection containing n-dimensional arrays will be converted to an (n + 1)-dimensional array. See [Java Collections](#java_collections) for more information.

named list of named vectors > data frame

empty arrays as lists...or empty collections as lists

empty map to empty list

row major is slower that column in df

see section below for conversion of bytes.
