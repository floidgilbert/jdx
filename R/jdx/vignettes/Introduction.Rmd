...
For those familar with the technical details of rJava, here are two important notes to consider. First, the jdx package behavior diverges from rJava for `raw` values and Java values of type `byte` and `java.lang.Byte`. Please see [R raw Values and Java byte Values](#R_raw_Java_byte) for more information. Second, for sites that plan on designing and distributing packages that depend on jdx, jdx does not use or load rJava's companion package JRI (the Java/R Interface).

## Primary Functions

The example in this section demonstrates the three primary functions in jdx: `convertToJava()`, `convertToR()`, and `getJavaClassName()`. First, create a 2 x 3 integer matrix:
...
Next, load the jdx library and use `convertToJava()` to convert the matrix to a two-dimensional Java array (`int[][]`). The resulting object is an rJava reference that points to the new Java object. The last line returns the R class information for the reference object.
...
The `getJavaClassName()` function returns the class name of the Java object. In this example, the clase name is `[[I`, Java's shorthand for `int[][]`.

```{r}
getJavaClassName(o)
```

Notice that the call to `Arrays.deepToString()` reveals that `convertToJava()` uses row-major ordering by default, but other ordering options are also available.///break this example into prose.///
...
Row names for data frames are not preserved during conversion. To include row names in the conversion, simply add them as a column in your data frame. We do not automatically include row names in conversion because it would require us to create an additional element in the Java map with a reserved key value such as `_row`. Instead, we leave the decision of how to handle row names up to the developer.
