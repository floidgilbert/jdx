...
The jdx package supports data exchange for matrices and other n-dimensional arrays. Three array index ordering schemes are available via the `convertToJava()` parameter `array.order`: `'row-major'`, `'column-major'`, and `'row-major-java'`. Confusingly, the terms *row-major* and *column-major* can be used to describe both array storage order and array index order. For example, R uses column-major array storage order: the elements in each column are stored consecutively in memory, and the elememts of one column have lower memory addresses than the elements of the next column. However, R language syntax uses row-major indexing. For a matrix `a`, the expression `a[1, 2]` returns the element in the first row and second column. That is, the row is represented by the first index.



///if you want Java to use a collection of rows, use row-major. if you want a collection of columns, use column-major. if you want large,smaller,smaller, use column-major.

///describe what happens on the way back from Java. The /// object in the array is considered a row in R when...

///check everything with jsonlite again.
...
Java n-dimensional arrays are literally constructed as collections of smaller structures as described in the preceding paragraph (see [Iliffe vectors](https://en.wikipedia.org/wiki/Iliffe_vector) for more information). R n-dimensional arrays, on the other hand, are stored in memory as a one-dimensional array and the data are retrieved in the appropriate order using indexing arithmetic.

///why are we talking about the terms here? need an intro sentence/// 

The `array.order` parameter of `convertToJava()` function refers strictly to the indexing scheme.

Confusingly, *array storage order* is not the same as *array indexing order*, though the same terminology can be applied to both. For example, R uses column-major array storage order: the elements in each column are stored consecutively in memory, and the elememts of one column have lower memory addresses than the elements of the next column. However, R language syntax uses row-major indexing. For example, a matrix array indexing order: the firs.   use the same terminology, but  can refer to the order array elements are stored in memory as well as the indexing scheme. R stores 


///rename row-major-java. it's just wrong all over the place.


///change all array.order to array.index.order

row-major gives you the same indexing as R. use this if you want it to be the same.but it's not.

Now we describe the each of the `array.order` options.

///https://en.wikipedia.org/wiki/Array_data_structure#Multidimensional_arrays under Compact layouts
