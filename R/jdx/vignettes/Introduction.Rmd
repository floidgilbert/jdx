...
**NOTE:** If `lst` is an unnamed list, `convertToR(convertToJava(lst))` may not result in an unnamed list. The `convertToJava()` function converts unnamed R lists to `ArrayList` objects which implement the `Collection` interface. The `convertToR()` function converts Java collections to the most appropriate R object depending on content (see the table below). For example, a collection containing compatible scalars will be converted to an R vector, and a collection containing n-dimensional arrays will be converted to an (n + 1)-dimensional array.
