...
The [jdx](https://cran.r-project.org/package=jdx) package builds on Simon Urbanek's [rJava](https://cran.r-project.org/package=rJava) package to simplify and extend data exchange between R and Java. The jdx package was originally developed to provide data exchange functionality between R and Java for the [jsr223](https://cran.r-project.org/package=jsr223) package, a high-level scripting interface for the Java platform. We provide jdx to developers who may want to extend existing rJava solutions. Developers of new applications are encouraged to use jsr223 for rapid application development with a relatively low learning curve.

The jdx package converts R data structures to generic Java objects and vice versa. In particular, R vectors, n-dimensional arrays, factors, data frames, tables, environments, and lists are converted to Java objects. Java scalars and n-dimensional arrays are converted to R vectors and n-dimensional arrays. Java maps and collections are converted to R lists, data frames, vectors, or n-dimensional arrays depending on content. Several options are available for data conversion including various orderings for arrays and data frames.

**DBD: Both of these next paragraphs are quite technical and feel out of place here.  I understand that you are trying to alert an rJava expert, but it will be lost of most people.  Maybe just combine the two paragraphs and add a leading sentence like, "For those familar with the technical details of rJava, we note two important aspects of the design of jdx that are discussed in more detail later.  First, ..."**
...
The example below demonstrates the three primary functions in jdx: `convertToJava()`, `convertToR()`, and `getJavaClassName()`.  **DBD:  Have you thought about using shorter function names toJava, toR, and javaClassName?**  **DBD: I find this next sentence confusing because there isn't a lot of context.  I suggest breaking up the example with prose designed to walk the reader through it.  This can include moving the R comments to manuscript text.** Notice that the call to `Arrays.deepToString()` reveals that `convertToJava()` uses row-major ordering by default, but row-major-java and column-major ordering is also available.  ***DBD: I'm confused about the three orderings.  I'd expect two orderings.  Perhaps you don't mention ordering here, or you just mention that it will be discussed later.**
...
**DBD: This seems too technical here while a reader is just trying to understand the usage of the package.**
...
**DBD: It's strikes be as weird that the first 'l' in 'lowLevel' is not capitalized.**

**NOTE:** When vectors or n-dimensional arrays are copied to Java, names are not preserved. Column names for data frames are preserved, but row names are not.  **DBD: I suppose this was a design choice, but some justification (either practical or technical) would be interesting.  Also, you could provide an example showing how to transfer names seperately as an array of strings.**
...
**DBD: The comments for the tables below look ugly.  Perhaps there is nothing you can do with R markdown to improve the look.  Or maybe you find another way to provide the comments outside of the table...**
...
**DBD: I think I am confused about row major vs. column major order.  People say that R is column major, whereas you imply below that R is row major.  See the first table after "Problem 1" at https://modelingguru.nasa.gov/docs/DOC-2625 **
...
The jdx package supports data exchange for lists, named lists, nested lists (i.e., lists containing other lists), and environments.  **DBD: Does that include lists of matrices, for example?**  The `convertToJava()` function converts R lists to Java `ArrayList<Object>` objects. It converts named lists and `environment` objects to Java `LinkedHashMap<String, Object>` objects. The `ArrayList` and `LinkedHashMap` objects implement the Java `Collection` and `Map` interfaces, respectively. These interfaces are ubiquitous in the Java API.
...
All of the primitive Java data types and their object, or boxed, counterparts are supported (e.g. `int` and `java.lang.Integer`). In addition, jdx supports `java.lang.String`, `java.math.BigDecimal`, and `java.math.BigInteger`. 
